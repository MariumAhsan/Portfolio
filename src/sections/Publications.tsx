...
function PublicationItem({ title, year, venue, authors, link, summary }: PublicationItemProps) {
  ...
  return (
    <article className="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-6 shadow-sm hover:shadow-md transition">
      <h3 className="text-lg font-semibold text-gray-900">{title}</h3>
      <p className="text-sm text-gray-500 mt-1">{year} • {venue}</p>
      <p
        className="text-sm text-gray-700 mt-2"
        dangerouslySetInnerHTML={{ __html: `Authors: ${formattedAuthors}` }}
      />
      {summary && <p className="text-sm text-gray-700 mt-2">{summary}</p>}
      <a
        href={link}
        target="_blank"
        rel="noopener noreferrer"
        className="mt-3 inline-flex items-center text-sm font-medium text-brand hover:underline"
      >
        View Paper &rarr;
      </a>
    </article>
